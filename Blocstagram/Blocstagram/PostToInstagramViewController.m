...
- (void)viewDidLoad {
    ...
    // QUESTION: when do you know when to reuse objects and use this construct?
...
- (PostCollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
...
//    UILabel *label = (UILabel *)[cell.contentView viewWithTag:labelTag];
    ...
    cell = [cell layoutWithEdgeSize:thumbnailEdgeSize];
...
- (void) addFiltersToQueue {
    ...
    // QUESTION so by defn, adding an operation to an operation Queue is run in background?
    // is concurrent synonymous with run in background
    ...
    [self.photoFilterOperationQueue addOperationWithBlock:^{
        ...
        if (drunkFilter) {
            ...
            if (tiltFilter) {
                ...
                [tiltFilter setValue:@0.2 forKeyPath:kCIInputAngleKey]; // value in radians
...
- (void) sendButtonPressed:(id)sender {
    ...
    if ([[UIApplication sharedApplication] canOpenURL:instagramURL]) {
        ...
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"No instagram app", nil) message:NSLocalizedString(@"The instagram app is not installed on your device. Please install from the app store.", nil) delegate:nil cancelButtonTitle:NSLocalizedString(@"OK", @"OK button") otherButtonTitles:nil];
        [alert show];
    }
}
